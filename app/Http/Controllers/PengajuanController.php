...
class PengajuanController extends Controller
{
    ...
    public function updateSurat(Request $request, $id_pengajuan)
    {
        try {
            // Validasi input
            $request->validate([
            ...
            $pengajuan->status = "Selesai"; // Pindah ke surat_selesai
            $pengajuan->save();

            // Generate PDF berdasarkan template yang sudah diedit
            $pdf = Pdf::loadView('pdf_template', compact('pengajuan'));

            // Simpan PDF
            $fileName = 'surat_' . $pengajuan->id_pengajuan . '.pdf';
            $pdfPath = public_path('storage/' . $fileName);
            $pdf->save($pdfPath);

            // Simpan path PDF ke database
            $pengajuan->update(['file_pdf' => $fileName]);
    ...
    public function editSurat($id_pengajuan)
    {
        $pengajuan = $this->pengajuan->pengajuan()
            ->where('pengajuan_surats.status', '=', 'Disetujui RW')
            ->where('pengajuan_surats.id_pengajuan', '=', $id_pengajuan) // Filter ID Pengajuan
        ...
        return view('edit_surat', compact('pengajuan'));
    }
    ...
    public function editPdf(Request $request, $id)
    {
        ...
        $pdfPath = public_path("pdfs/{$id}_pengantar_rt.pdf");
